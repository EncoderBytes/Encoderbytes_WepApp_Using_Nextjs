...
export async function GET(request, { params }) {
  try {
    ...
    const id = params.approachID;

    const [rows] = await db.query("SELECT * FROM ourapproaches WHERE id = ?", [id]);

    if (rows.length === 0) {
      return NextResponse.json({ result: "No Approaches Data Available" });
    }

    return NextResponse.json({ Result: rows[0] });
  } catch (error) {
    console.error("Error fetching approahces:", error);
...
export async function DELETE(request, { params }) {
  try {
    ...
    const oldPublicId = approach.publicId;

    // Delete from MySQL
    const [deleteResult] = await db.query(
      "DELETE FROM ourapproaches WHERE id = ?",
      [id]
    );

    if (deleteResult.affectedRows === 0) {
      return NextResponse.json({
        message: "Failed to delete approach",
        status: 500,
      });
    }
...
export async function PUT(request, { params }) {
  try {
    ...
    // Extract fields
    const { no, heading, description } = Object.fromEntries(
      form.entries()
    );
    ...
    const [updateRes] = await db.query(
      `UPDATE ourapproaches 
       SET no=?, heading=?, description=?, image=?, publicId=? 
       WHERE id = ?`,
      [
        no || old.no,
        heading || old.heading,
